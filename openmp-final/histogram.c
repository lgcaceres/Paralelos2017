...
void Fill(float min_meas,float max_meas, float data[], int data_count);

void Bins(float min_meas, float max_meas, float bin_maxes[], int bin_counts[], int bin_count);

int Choose(float data, float bin_maxes[], int bin_count, float min_meas);
...
int main(int argc, char* argv[]) 
{
	...
	Fill(min_meas, max_meas, data, data_count);

	Bins(min_meas, max_meas, bin_maxes, bin_counts, bin_count);
    ...
    {
    	bin = Choose(data[i], bin_maxes, bin_count, min_meas);
    	#pragma omp critical
    	bin_counts[bin]++;
  	}
